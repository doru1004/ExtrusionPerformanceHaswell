...
adds_to_muls = {}
adds_to_muls["CG1xCG1"] = 1.68
adds_to_muls["CG1xDG0"] = 1.66
adds_to_muls["CG1xDG1"] = 1.68
adds_to_muls["DG0xCG1"] = 1.66
adds_to_muls["DG0xDG0"] = 1.63
adds_to_muls["DG0xDG1"] = 1.66
adds_to_muls["DG1xCG1"] = 1.69
adds_to_muls["DG1xDG0"] = 1.66
adds_to_muls["DG1xDG1"] = 1.69
...
if form_side == "FRHS":
    # This is for fvdx.
    adds_to_muls = {}
    adds_to_muls["CG1xCG1"] = 1.76
    adds_to_muls["CG1xDG0"] = 1.97
    adds_to_muls["CG1xDG1"] = 1.76
    adds_to_muls["DG0xCG1"] = 1.87
    adds_to_muls["DG0xDG0"] = 1.66
    adds_to_muls["DG0xDG1"] = 1.87
    adds_to_muls["DG1xCG1"] = 1.76
    adds_to_muls["DG1xDG0"] = 1.97
    adds_to_muls["DG1xDG1"] = 1.76
...
class GeneratePlots(object):
    ...
    vset2str = ["MPI Processes", "Layers",                # 0, 1
                ...
                "Valuable BW [MB/s]",                     # 10
                "Non-ordered Valuable BW [MB/s]",         # 11
            ...
            for line in f1:
                ...
                if "base" in line:
                  ...
                  if not reduction_record.isEmpty():
                    ...
                    # Transform the number of cycles into seconds
                    reduction_record = ReductionRecord()
                  if "layers" in line:
                    layers = int(words[2])
            ...
            for filename in os.listdir(extra_res_dir):
                with open(extra_res_dir+"/"+filename) as h:
                    if "Memory BW [MBytes/s]" in line:
                        ...
                        value_found = max(value_found, float(line.split()[6]))
            if field_name == "Memory BW [MBytes/s]":
                        ...
                        if ds[self._vset2ds[l_idx]] == l and (l in l_set or l_set == []):
                            ...
                            for x_val in self._vset[x_idx]:
                                if ds[self._vset2ds[x_idx]] == x_val and x_val == x_set[0]:
                                    factor = 2.0 if x_val == 32 else 1.0
                                    res = []
...
# 15 self.papi_mflops
# 16 self.cycles
measures = [3, 4, 8, 9, 10, 11, 12, 14, 16]
mem_set = [8, 9, 10, 11]
...
print_table([A2M_RATIO, A2M_GFLOPS, GFLOPS, A2M_PERC_PEAK, VBW_PEAK], mpi_ps=[32], layer_list=[100])

print "CUSTOM VECT ADDS TO MULS TABLE"
print_table([A2M_RATIO, VECT_RATIO, VECT_A2M_PERC_PEAK, VBW_PEAK], mpi_ps=[32], layer_list=[100])
