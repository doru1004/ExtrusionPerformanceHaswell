...
# For extruded meshes
# Kernel CG1xCG1_1 Real_time 0.0119426 Proc_time 0.0119426 Total_instructions 0 IPC 0 Total_flpins 12.7448 MFLOPS 1.07161 DV 76.8288 proc 16
...
PEAK_BW_BS=80973.6 / div_factor *1024*1024 # B/s
PEAK_BW_GBS= 80973.6 / div_factor / 1024 # GB/s

# BW provided by Intel
PEAK_BW_BS_Intel=59000.0*SOCKETS*1024*1024 # B/s
PEAK_BW_GBS_Intel= 59000.0*SOCKETS / 1024 # GB/s
ASPECT_RATIO=1
...
#formatting:
ax.set_title("E5-2640 Xeon Haswell v3 - %(type)s" % {'type': ""}, fontsize=fontsize0, fontweight='bold')
...
if actual_cores == 1:
    ...
    if vectorized:
        Y_MAX = SP_FP_balance * xdist * 1.20
    else:
        Y_MAX = 2.0 * SP_multicore * xdist * 1.20
...
#formatting:
ax.set_title("E5-2640 Xeon Haswell EP v3 %(type)s" % {'type': ""}, fontsize=fontsize0, fontweight='bold')
ax.set_xlabel("Operational Intensity [FLOPs/Byte]", fontsize=fontsize1)
ax.set_ylabel("Performance [FLOPS]", fontsize=fontsize1)
...
if type_of_test.startswith("RHS"):
...
if actual_cores == 1 and vectorized:
    ...
    ax.text(pos, 8.0*SP_singleCore * xdist, "SIMD + FPB ("+str(float(SP_singleCore) * 8.0/pow(10,9))+" GFLOPS)", fontsize=fontsize2, alpha=alpha_gflops)
...
if Y_MIN < 2.0 * SP_multicore and actual_cores > 1:
    ...
    ax.text(pos, 2.0 * SP_multicore * xdist, "TLP + FPB ("+str(float(SP_multicore) * 2.0/pow(10,9))+" GFLOPS)", fontsize=fontsize2, alpha=alpha_gflops)
...
if actual_cores > 1 and vectorized:
    ...
    ax.text(pos, SP_FP_balance * xdist, "SIMD + FPB ("+str(float(SP_FP_balance)/pow(10,9))+" GFLOPS)", fontsize=fontsize2, alpha=alpha_gflops)
...
#formatting:
ax1.set_title("E5-2640 Xeon Haswell v3 %(type)s" % {'type': ""}, fontsize=fontsize0, fontweight='bold')
...
for variable in x:
    ...
    for var2 in range(0, len(x[variable])):
        ...
        if int(x[variable][var2][2]) == 0:
            ...
            # ax1.annotate("", xy=(aux_layers_x[var2], aux_layers_y[var2]), xycoords='data', xytext=(-15+xOffset, -10+yOffset), textcoords='offset points', fontsize=fontsize4) 
        else:
            ax1.scatter([aux_layers_x[var2],],[aux_layers_y[var2],], s=80, color=x[variable][0][3], alpha=1.0)
            # ax1.annotate(x[variable][var2][2], xy=(aux_layers_x[var2], aux_layers_y[var2]), xycoords='data', xytext=(-15+xOffset, -10+yOffset), textcoords='offset points', fontsize=fontsize4) 
...
if Y_MIN < 2.0 * SP_singleCore:
    ...
    ax1.text(pos, 2.0*SP_singleCore * xdist, "BALANCED ("+str(float(SP_singleCore) * 2.0)+" GFLOPS)", fontsize=fontsize2, alpha=low_alpha)
...
if actual_cores == 1 and vectorized:
    ...
    ax1.text(pos, 8.0*SP_singleCore * xdist, "SIMD + FPB ("+str(float(SP_singleCore) * 8.0)+" GFLOPS)", fontsize=fontsize2, alpha=low_alpha)
...
if Y_MIN < 2.0 * SP_multicore and actual_cores > 1:
    ...
    ax1.text(pos, 2.0 * SP_multicore * xdist, "BALANCED ("+str(float(SP_multicore) * 2.0)+" GFLOPS)", fontsize=fontsize2, alpha=low_alpha)
...
if actual_cores > 1 and vectorized:
    ...
    ax1.text(pos, SP_FP_balance * xdist, "SIMD + FPB ("+str(float(SP_FP_balance))+" GFLOPS)", fontsize=fontsize2, alpha=low_alpha)
